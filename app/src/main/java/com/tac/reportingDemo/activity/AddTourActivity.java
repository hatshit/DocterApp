...
//                dialog.getDatePicker().setMinDate(1546300800000L);
    ...
    private void checkDataAndSubmitTour() {
        String toDate, fromDate, place, remark;
        ...
        checkNetworkAndPlanTour(place, fromDate, toDate, remark);

    }

    private void checkNetworkAndPlanTour(String place, String fromDate, String toDate, String remark) {
        if (Utils.isNetworkAvailable()) {
            submitTourPlan(place, fromDate, toDate, remark);
    ...
    private void submitTourPlan(String place, String fromDate, String toDate, String remark) {
        Utils.showPB(mParent, mPb);

        StringRequest request = new StringRequest(Request.Method.POST, ENDPOINTS.SUBMIT_REPORTS, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                Utils.hidePB(mParent,mPb);
                try {
                    if (JsonParsor.isReqSuccesful(response)) {
                        Utils.makeToast("Tour Plan Submitted");
    ...
    private void setToolbar() {
        ...
        getSupportActionBar().setTitle("Plan Tour");
